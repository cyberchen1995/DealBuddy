...
- **无障碍收口(前端迭代 goal 收官轮)**:
  - 对比度全量审计(亮暗两套全部实际配对):亮色次要文字加深为 `#7a7060`
    (白卡 4.87)、亮色强调色加深为 `#a84f2e`(白卡 5.49,审计新发现),
    与插件 tokens.css 同步;暗色套全部达标;唯一残余(软底上次要字 4.26)
    经复审接受并记录于 tokens.md。
  - 顶栏服务/会话/LLM 状态变化增加读屏播报(aria-live)。
  - 全键盘走查通过:Tab 序自然,收起卡片内容按原生语义跳过,
    弹窗焦点循环 / toast 键盘关闭 / 确认层在此前版本已具备。
