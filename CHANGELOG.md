...
- 商品列表可视高度不再依赖「页头恒为 170px」的硬编码假设:页头实际高度
  (含窄幅换行)由 ResizeObserver 写入 CSS 变量,列表高度随之自适应。
  1200/780/500 三档断点与确认层、toast 的窄幅表现走查通过。

## [0.9.2] - 2026-08-14

### Changed

- **首屏骨架屏**:会话列表、报告区、商品列表在数据加载前显示占位骨架
  (随系统明暗,`prefers-reduced-motion` 下静止);加载失败回退到空状态,
  错误经 toast 提示。
- **进行中反馈统一**:重新生成报告与测试连接按钮增加呼吸 busy 态,
  报告重生成期间旧内容半透明保持可读。
- 空状态文案微调(dealbuddy-copywriter 审校):商品空态去掉冗余前提句,
  报告占位统一为「暂无报告/采集商品后自动生成。」并修复首次渲染后占位丢失。
...
- 删除商品的确认从浏览器原生弹窗改为工作台内**确认层**:显示具体商品名与后果说明
  （报告会重新生成），焦点落在「删除」上与原生 Enter 习惯一致，Esc / 点遮罩 /
  「取消」均可退出，键盘 Tab 在两个按钮间循环，关闭后焦点回到触发按钮。
- 删除商品接口改为**按商品链接定位**（`DELETE /api/sessions/{id}/offers?url=…`，
  服务端写锁内解析位置），旧的按下标接口移除：确认弹窗等待期间的重采集重排
  不再可能误删其它商品。
