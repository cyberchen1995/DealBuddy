"""五处版本号一致性:pyproject / 扩展 manifest / __init__ / web.py(FastAPI 与 MCP serverInfo)。
...
def test_version_synchronized() -> None:
    ...
    manifest = json.loads(
        (ROOT / "extension" / "dealbuddy-capture" / "manifest.json").read_text(encoding="utf-8")
    )
    assert manifest["version"] == expected, "extension/dealbuddy-capture/manifest.json 版本未同步"

    assert dealbuddy.__version__ == expected, "src/dealbuddy/__init__.py __version__ 未同步"

    web_source = (ROOT / "src" / "dealbuddy" / "web.py").read_text(encoding="utf-8")
    fastapi_versions = re.findall(r'FastAPI\(title="DealBuddy", version="([^"]+)"\)', web_source)
