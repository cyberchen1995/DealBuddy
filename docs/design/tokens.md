...
对比度审计记录(A1 收官轮 2026-08-14 全量复审;线:正文 ≥4.5 / 按钮等大字重组件 ≥3):

- 暗色套全部实际配对达标(最低 5.0:1)。
- 亮色主按钮(≈3.07)与危险按钮 hover(≈3.97)按组件线合规
  (2026-08-12 用户决策保留浅陶土主色)。
- 亮色 `--db-muted` 已由 `#8f8577` 加深为 `#7a7060`(白卡 4.87 / 浮起卡 4.79 /
  页面底 4.52);**唯一残余**:软底 `--db-surface-soft` 上 ≈ 4.26,低于正文线,
