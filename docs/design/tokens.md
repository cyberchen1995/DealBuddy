...
对比度审计记录(A1 收官轮 2026-08-14 全量复审)。口径:WCAG 正文线为 4.5:1;
「按钮等大字重组件 ≥3:1」是本项目 2026-08-12 用户拍板的自定线(工作台按钮文字为
13.5px/600,按 WCAG 口径仍属正文,不构成 WCAG 意义上的合规)。

- 暗色套全部实际配对达标(最低 5.0:1),无例外。
- 亮色 `--db-muted` 已由 `#8f8577` 加深为 `#7a7060`(白卡 4.87 / 浮起卡 4.79 /
  页面底 4.52);`--db-accent-strong` 已由 `#c2603f` 加深为 `#a84f2e`
  (白卡 5.49 / 选中底 4.65);选中会话项元数据改用 `--db-muted-strong`
  (`accent-soft` 底上 5.13)。

**已接受的文字对比度例外(WCAG 4.5:1 口径,亮色套,共三条)**:

1. 主按钮白字 on `--db-accent` ≈ 3.07(满足项目组件线;2026-08-12 用户看对比
   demo 后决策保留浅陶土主色)。
2. 危险按钮 hover 白字 on `--db-danger` ≈ 3.97(同上,danger 静默态为深字浅底
   5.0+,仅 hover 反色瞬态低于正文线)。
3. `--db-muted` on `--db-surface-soft` ≈ 4.26(软底容器内 12px 辅助字,
   2026-08-14 用户复审接受)。
